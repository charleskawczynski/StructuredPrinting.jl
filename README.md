...
@structured_print t PrintingOptions(typeof(t.branchB))

# Print struct with Tuple of types highlighted
@structured_print t PrintingOptions((typeof(t.branchB), typeof(t.branchA)))
```

In addition, there is a special printing option, called `GoodProperties()`, which can be helpful to find which object may cause inference issues:
...
using StructuredPrinting
@structured_print obj GoodProperties() # see who is responsible
```
